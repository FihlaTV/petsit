...
knex.raw(`SELECT distinct on (pets.id) requests.*, users.username, pets.pet_name, pets.id as pet_id, pet_reviews.rating, pets.pic_url FROM requests JOIN users ON requests.poster_id = users.id JOIN pets ON pets.id = requests.pet_id JOIN pet_reviews ON pet_reviews.pet_id = pets.id;`)
