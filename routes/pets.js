...
router.get('/:id', function(req, res, next) {
  knex.raw(`select pets.*, users.username, users.location from pets join users on pets.owner_id = users.id WHERE pets.id = ${req.params.id}`)
  .then(function(pets){
    ...
    .then(function(reviews) {
        ...
        .then(function(ave){
          ...
          .then(function(ratings){
            res.render('petShow', {pet: pets.rows[0], cookie: req.cookies.user_id, reviews: reviews.rows, starAve: Math.round(ave.rows[0].avg), username: req.cookies.username, ratings:ratings.rows});
          })
